...
class ExportToExcel extends Action implements FromQuery, WithCustomChunkSize, WithHeadingsConcern, WithMapping {
  ...
  public function map($row): array {
    ...
    if (isset($row["user_id"])) {
      ...
      $row["user_id"] = $user->name . " (ID: " . $user->id . ", phone: " . $user->phone . ", email: " . $user->email . ")";
    }
    if (isset($row["topic_id"])) {
      $row["topic_id"] = Topic::find($row["topic_id"])->name. " (ID: " . $row["topic_id"] .")";
    }
    if (isset($row["event_id"])) {
      $row["event_id"] = Event::find($row["event_id"])->name. " (ID: " . $row["event_id"] .")";
    }
    if (isset($row["file"])) {
      if ($row["file"]){
        $row["file"] = env('APP_URL')."/storage/reports/".$row["file"];
      }
    }
